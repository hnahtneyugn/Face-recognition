...
async def get_daily_attendance(
    ...
    print(users)
    attendances = []
    seen = set()  # Dùng để track các bản ghi đã thấy
    
    for user in users:
        user_attendances = await Attendance.filter(user=user).order_by("-date").prefetch_related("user")
        for att in user_attendances:
            # Tạo key duy nhất cho mỗi cặp user-date
            unique_key = (att.user.user_id, att.date)
            if unique_key not in seen and attendance_filters(att, year, month, day, status):
                seen.add(unique_key)
