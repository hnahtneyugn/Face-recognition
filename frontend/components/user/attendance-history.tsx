...
export default function AttendanceHistory({ userId, refreshTrigger = 0 }: AttendanceHistoryProps) {
  ...
  useEffect(() => {
    const fetchAttendanceHistory = async () => {
      try {
        ...
        if (filterType === "day") {
          if (!confirmedDate || !validDate) return;
  ...
  }, [userId, refreshTrigger, filterType, monthFilter, yearFilter, confirmedDate])
  ...
  const handleDateChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const isValid = !isNaN(date.getTime()) && value.length === 10; // Đảm bảo đã nhập đủ YYYY-MM-DD
  ...
  const handleDateKeyDown = (e: React.KeyboardEvent<HTMLInputElement>) => {
    if (e.key === 'Enter') {
      ...
      // Kiểm tra tính hợp lệ của ngày khi ấn Enter
      const date = new Date(value);
      const isValid = !isNaN(date.getTime()) && value.length === 10;

      if (isValid) {
        setConfirmedDate(value); // Chỉ cập nhật confirmedDate khi ngày hợp lệ
      } else {
        setAttendanceRecords([]); // Xóa kết quả nếu ngày không hợp lệ
      }
    }
  }
