...
import { Camera, Loader2, AlertTriangle, Info } from "lucide-react"
import { Alert, AlertDescription, AlertTitle } from "@/components/ui/alert"
import { Tabs, TabsContent, TabsList, TabsTrigger } from "@/components/ui/tabs"
import { loadModels, detectFacesInVideo } from "@/lib/face-api"
  ...
  const handleCaptureImage = async () => {
    ...
    try {
      ...
      if (!blob || blob.size === 0) {
        setAttendanceResult({
          success: false,
          message: "Không thể chụp ảnh khuôn mặt. Vui lòng thử lại.",
        });
      ...
      const result = await apiClient.post('/users/attendance', formData) as { time?: string };
