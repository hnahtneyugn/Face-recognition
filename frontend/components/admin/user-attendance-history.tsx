...
export default function UserAttendanceHistory({
  ...
  // State để lưu ngày đã xác nhận
  const [confirmedDate, setConfirmedDate] = useState<string>("");
  // State để lưu trạng thái hợp lệ của ngày
    ...
    const fetchAttendanceHistory = async () => {
      try {
        ...
        const data = await api.get<AttendanceRecord[]>(
          `admins/attendance/${userId}${queryString ? `?${queryString}` : ""}`
        );
  ...
  }, [userId, filterType, confirmedDate, monthFilter, yearFilter, validDate, toast]);
  ...
  const handleBack = () => {
    if (source === "daily") {
      router.replace(`/admin/attendance?date=${savedDate}`);
    } else {
