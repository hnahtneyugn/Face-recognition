...
export default function DailyAttendanceList() {
  ...
  // Lấy ngày từ URL nếu có (khi quay lại từ trang lịch sử), nếu không thì dùng ngày hiện tại
  ...
  // Thêm useEffect để xử lý nút back của trình duyệt
  useEffect(() => {
    // Thay thế URL hiện tại bằng /admin khi component mount
    // Điều này sẽ đảm bảo khi bấm back sẽ về trang admin
  ...
  useEffect(() => {
    const fetchDailyAttendance = async () => {
      setIsLoading(true);
      try {
        // Tách ngày tháng năm từ confirmedDate
  ...
  }, [confirmedDate, searchTerm, departmentFilter, statusFilter, toast, router]);
  ...
  const handleDateKeyDown = (e: React.KeyboardEvent<HTMLInputElement>) => {
    if (e.key === 'Enter') {
      ...
      const isValid = !isNaN(date.getTime()) && value.length === 10;
  ...
  const handleViewUserAttendance = (userId: string, userName: string) => {
    // Sử dụng push để thêm vào history stack
    router.push(
      `/admin/user-attendance/${userId}?name=${encodeURIComponent(userName)}&source=daily&date=${encodeURIComponent(selectedDate)}`
    );
  };
