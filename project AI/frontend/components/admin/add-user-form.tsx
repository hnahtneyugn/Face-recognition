...
export default function AddUserForm({ onUserAdded }: AddUserFormProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_BASE_URL || "http://localhost:8000"}/admins/`, {
        method: "POST",
        headers: {
      ...
      if (!response.ok) {
        ...
        throw new Error(errorData.detail || "Lỗi khi thêm người dùng");
      }
      ...
      toast({
        title: "Thành công",
        description: `Đã thêm người dùng ${data.fullname}`,
      });
